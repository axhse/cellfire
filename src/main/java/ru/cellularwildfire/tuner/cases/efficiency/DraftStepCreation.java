package ru.cellularwildfire.tuner.cases.efficiency;
...
public final class DraftStepCreation extends TuneCase {
        ...
        .forEach(
            cell -> {
              ...
              Cell.State draftCellState =
                  new Cell.State(lastCellState.getHeat(), lastCellState.getFuel(), true);
                ...
                for (int offsetY = -1; offsetY <= 1; offsetY++) {
                  ...
                  draftStep
                      ...
                      .forEach(
                          otherCell -> {
                            int distanceX =
                                otherCell.getCoordinates().getX()
                                    - neighbor.getCoordinates().getX();
                            int distanceY =
                                otherCell.getCoordinates().getY()
                                    - neighbor.getCoordinates().getY();
  ...
  public void assess(ThermalAlgorithm algorithm, Assessment assessment) {
    Map<CopyingAlgorithm, Function<Simulation.Step, Simulation.Step>> creators =
        Map.of(
    ...
    for (Map.Entry<CopyingAlgorithm, Function<Simulation.Step, Simulation.Step>> entry :
        creators.entrySet()) {
